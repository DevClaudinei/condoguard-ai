...
Este documento é Markdown puro com *front matter* YAML, pronto para exportação via [Pandoc](https://pandoc.org/). As figuras estão em `docs/img/` e são referenciadas por caminho relativo (URL-encoded), o que é resolvido automaticamente pelo Pandoc.

**PDF via Typst** (recomendado — leve, sem TeX, balanceamento automático de viúvas/órfãs):

    pandoc docs/ENTREGA_FASE1.md -o docs/ENTREGA_FASE1_FINAL.pdf --pdf-engine=typst --resource-path=docs

**PDF via LaTeX** (requer `texlive`/`xelatex`):
